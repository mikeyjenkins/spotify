...
const Player = () => {
  ...
  const renderPlayPauseBtn = () => {
      if(track && !track.is_playing){
        return <FontAwesomeIcon className='play-track-icn' icon={faPlay} onClick={() => playTrack()} />
      }
      else if(track) {
        return <FontAwesomeIcon className='play-track-icn' icon={faPause} onClick={() => pauseTrack()} />
      }
  }
          ...
          {track && (
            <div className="current-song-wrapper">
              ...
              <div className="track-control-btns">
                <FontAwesomeIcon className='prev-track-arrow' icon={faBackward} onClick={() => prevTrack()} />
                {renderPlayPauseBtn()}
                <FontAwesomeIcon className='next-track-arrow' icon={faForward} onClick={() => nextTrack()} />
              </div>
